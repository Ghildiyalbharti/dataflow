...
class ComputeAvgCarryForward(beam.DoFn):
    """
    Compute averages per window, but if the window has
    no elements, reuse the last window’s result.
    """
    def __init__(self):
    ...
    def process(self, window_key_values):
        ...
        if count == 0 and self.last_result:
            # Window is empty → reuse last emitted
            new_result = dict(self.last_result)
            new_result['window'] = window_key
            yield window_key, new_result
...
def run(input_topic, output_path, window_size, pipeline_args=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        (
            ...
            | "Compute Average" >> beam.ParDo(ComputeAvgCarryForward())
